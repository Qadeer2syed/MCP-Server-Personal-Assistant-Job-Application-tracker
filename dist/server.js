...
class PersonalAssistantMCPServer {
    ...
    setupGoogleAuth() {
        this.googleAuth = new OAuth2Client(this.OAUTH_CONFIG.CLIENT_ID, this.OAUTH_CONFIG.CLIENT_SECRET, this.OAUTH_CONFIG.REDIRECT_URI);
    ...
    startServerOnAvailablePort(server, startPort, callback) {
            ...
            try {
                switch (name) {
                    ...
                    case 'get_upcoming_meetings':
                        return await this.getUpcomingMeetings(args);
                    case 'check_emails':
                        return await this.checkEmails(args);
                    case 'add_job_application':
                        return await this.handleAddJobApplication(args);
                    case 'update_job_status':
                        return await this.handleUpdateJobStatus(args);
                    case 'list_applications':
                        return await this.handleListApplications(args);
    ...
    async handleAddJobApplication({ spreadsheetId, jobTitle, company, dateApplied, status = 'Applied' }) {
        if (!(await this.ensureLoggedIn())) {
    ...
    async handleUpdateJobStatus({ spreadsheetId, jobTitle, newStatus }) {
        if (!(await this.ensureLoggedIn())) {
            return { content: [{ type: 'text', text: '❌ Please login first via login_google.' }], isError: true };
        }
        // 1. Fetch all rows
        ...
        const rowIndex = rows.findIndex((r) => r[0] === jobTitle);
    ...
    async handleListApplications({ spreadsheetId }) {
        if (!(await this.ensureLoggedIn())) {
            return { content: [{ type: 'text', text: '❌ Please login first via login_google.' }], isError: true };
        }
        const resp = await this.sheets.spreadsheets.values.get({
